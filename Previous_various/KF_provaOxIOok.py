...
parser.add_argument('--dset_type', type=string, required=True,default="oxford", help="oxford or matlab")
...
A = np.identity(3)   ##domanda: anche qui si puo' usare sia lista di liste che np.array ??
A = A.tolist()

# matrice di controllo 3x3: NON LA USO PERCHE' u e' 0 e quindi anche B*u=0. useless matrix :p
B = np.identity(3)  ##domanda: anche qui si puo' usare sia lista di liste che np.array ??
B = B.tolist()

# matrice C di osservazioni 6x3 IN ALTRI PAPERS SI CHIAMA H
C = [[1, 0, 0],
     [0, 1, 0],
     [0, 0, 1],
     [dt, 0, 0],
